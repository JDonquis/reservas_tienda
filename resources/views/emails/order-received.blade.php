                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px; width:100%; background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:12px; overflow:hidden;">
                    <tr>
                        <td style="padding:32px;">
                            <h1 style="margin:0 0 8px; font-size:22px; color:#0F172A;">¡Nuevo pedido recibido! 🛍️</h1>
                            <p style="margin:0 0 24px; font-size:14px; line-height:1.6; color:#475569;">
                                Pedido <strong>#{{ $order->id }}</strong> de {{ $order->customer_name }} en {{ $store->name }}.
                            </p>
                            ...
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#F8FAFC; border:1px solid #E2E8F0; border-radius:8px; margin-top:24px;">
                                <tr>
                                    <td style="padding:16px 20px; font-size:14px; color:#0F172A;">
                                        ...
                                        <strong>Entrega:</strong> {{ $order->shipping_method === 'delivery' ? 'A domicilio' : 'Retiro en tienda' }}<br>
                                        @if ($order->shipping_method === 'delivery')
                                            <strong>Dirección:</strong> {{ $order->shipping_address }}<br>
                                        @endif
                                        @if ($order->shipping_notes)
                                            <strong>Notas:</strong> {{ $order->shipping_notes }}<br>
                                        @endif
                                    </td>
                                </tr>
                            </table>
